...
* rainfall: Cumulative rainfall in the month leading up to the measurement
* oxygen: Oxygen saturation
* area: area of farmland divided by total upstream area. 0 --> no farming,
1 --> all farmland
* emission: emmission of N by animals divided by area of farmland
* stockage: stockage of manure in a year
...
* norm_N: threshold of N to be applied on the field
* depostion: depostion of N from the air in the total upstream area.
...
qplot(x = nitrate, data=nc, binwidth=1) + 
  geom_segment(aes(x =11, y = 0, xend = 11, yend = 400),  linetype="dashed",
               color="red")
...
Oxygen has a lot of 0 measurements. These are measurements that are below the
detection limit. This is important to remember once we start building our model.
...
These are removed from the dataset for further analysis.
...
First we want to investigate the trend in nitrate conccentrations in time.
...
concentrations have been decreasing. Also the number of exceedances (nitrate
concentrations > 11.3 mg N/L) has been decreasing in the last years.
...
The resulting plots and tables do show an effect of the catch crop.
...
total_veg & catchcrop. This last variable did not bring in additional
information and is left out of the final model.
...
in the upstream area and the nitratte concentrations measured in the river. 
...
includes all relevant ariables that were included in the analysis. This plot
shows that we can explain some of the observed variance based on these variables
but definetly not all.
...
variance based on the variables in our dataset but definetly not all.
...
well. It might be worth repeating this analysis with those variables included.
